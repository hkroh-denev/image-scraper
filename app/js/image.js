...
var $brightness_range = $('#brightness_range');
var $contrast_range = $('#contrast_range');
...
var taskSaveFilePath = path.join(__dirname, settings.System.Directories.config, 'last_task.json'); 
...
function loadJimpImage(img)
{
	if (img) {
		img.write(workingFilePath, function() {
				imageElementReset($current_image);
				$current_image.attr('src', 'working/' + imageInfo.imageTitle + '_work.jpg?timestamp=' + new Date().getTime());
				setDisplayRatio($current_image, img.bitmap.width, img.bitmap.height);
				$current_image.show();
				$statusbar.html('Ready');
		});
	}
...
{
	var info = {
		input_directory: path.dirname(imageInfo.original_path),
		last_index: imageIndex,
		last_filename: imageFiles[imageIndex]
	};
	fs.writeFileSync(taskSaveFilePath, JSON.stringify(info), 'utf-8');
}

function init()
{
	var localpath = remote.getGlobal('localpath');
	var lastTask = remote.getGlobal('lastTask');
	...
	if (lastTask) {
		if (imageFiles[lastTask.last_index] === lastTask.last_filename) {
			imageIndex = lastTask.last_index + 1;
		}
		else {
			imageIndex = imageFiles.indexOf(lastTask.last_filename) + 1;
		}
	}
	else {
		imageIndex = 0;
...
$image_area.mouseup(function(e) {
	...
	if (dragInfo.isDrag) {
		...
		Jimp.read(workingFilePath).then(function(im) {
			...
			if (new_image) {
				imageInfo.jimp_image = new_image;
				loadJimpImage(new_image);
			}
			else {
...
var control_brightness_and_contrast = function() {
	var val = parseInt($brightness_range.val());
	val = (val - 50) / 50;
	imageInfo.adjust_image = imageInfo.jimp_image.clone();
	imageInfo.adjust_image.brightness(val);
	val = parseInt($contrast_range.val());
	val = (val - 50) / 50;
	imageInfo.adjust_image.contrast(val);
	loadJimpImage(imageInfo.adjust_image);
};

$brightness_range.change(control_brightness_and_contrast);
$contrast_range.change(control_brightness_and_contrast);
$reset_button.click(function() {
	$brightness_range.val(50);
	$contrast_range.val(50);
	loadJimpImage(imageInfo.jimp_image);
});
$apply_button.click(function() {
	imageInfo.jimp_image = imageInfo.adjust_image;
	$brightness_range.val(50);
	$contrast_range.val(50);
	loadJimpImage(imageInfo.jimp_image);
});
...
$(window).keydown(function(ev) {
	switch(ev.keyCode) {
		...
		case keycode.RightKey:	// Right
			workingFilePath = path.join(workingDirectory, imageInfo.imageTitle + '_work.jpg');
			if (ev.keyCode === keycode.RightKey) {
